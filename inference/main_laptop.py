...
# ================== 3. SHARED STATE GIỮA CÁC THREAD ==================
...
# ================== 4. THREAD CAPTURE: luôn giữ frame mới nhất ==================
...
# ================== 5. THREAD XỬ LÝ: detect + track + OCR, chạy hết tốc lực GPU ==================
def processing_thread_func():
    ...
    # đếm riêng FPS xử lý thực tế, in ra console để so sánh với FPS hiển thị
    ...
    while not stop_event.is_set():
        with frame_cond:
            got_new = frame_cond.wait_for(
                lambda: stop_event.is_set() or (frame_ready and frame_version != last_processed_version),
                timeout=1.0
            )
            ...
            if r.boxes.id is not None:
                ...
                for box, tid in zip(boxes, track_ids):
                    ...
                    frame_results.append({
                        "box": (x1, y1, x2, y2),
                        "tid": tid,
                        "text": text if text else f"ID:{tid} (dang doc...)"
                    })
...
# ================== 6. MAIN THREAD: hiển thị ==================
def main():
    global detector, ocr_model
    t_capture = threading.Thread(target=capture_thread_func, args=('b.mp4',), daemon=True)
    ...
    while not stop_event.is_set():
        with frame_cond:
            got_new = frame_cond.wait_for(
                lambda: stop_event.is_set() or (frame_ready and frame_version != last_shown_version),
                timeout=1.0
            )
            if stop_event.is_set():
                break
        ...
        for res in results_to_draw:
            ...
            cv2.putText(display_frame, res["text"], (x1, y1 - 10),
        ...
        cv2.putText(display_frame, f"FPS: {int(smoothed_fps)}", (20, 50),
